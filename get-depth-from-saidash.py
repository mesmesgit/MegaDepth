...
#
#   Script to run MegaDepth network and generate depth images for all frames in Webots output
#
import os
import torch
import torchvision
...
def main():
    ...
    #
    # init paths
    run_id = 'saidash135_001'
    run_rootpath = os.path.join(rootPath, "imdata/pcs/" + run_id + "/")
    pspnetpath = os.path.join(rootPath, 'pspnet-pytorch/')
    pspnetconfig = os.path.join(pspnetpath, 'config/ade20k.yaml')
    out_class_figure = os.path.join(run_rootpath, "Run/semseg_out_figure.png")
    run_semseg = False
    # init count
    count = 1
    while True:
        try:
            ...
            #  get the path to the rgb file
            rgbPath = os.path.join(run_rootpath, "Run/rgb{0:06d}.png".format(count))
            ...
            if run_semseg:
                ...
                try:
                    ...
                    masked_image_path = os.path.join(run_rootpath, "Run/masked{0:06d}.png".format(count))
    ...
    run_params_filename = os.path.join(run_rootpath, "run_params_depth.npz")
    num_frames = count - 1
